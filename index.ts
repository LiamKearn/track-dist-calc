...
function nearestNth(value: number, divisor: number): number {
...
// TYPES AND MAPPINGS
// =============================================================================
type LaneNumber = 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8;
...
type OptimizedLanePlan = LanePlan & {
    route: RoutePlan;
};

type AnyLanePlan = LanePlan | OptimizedLanePlan;

function isOptimized(plan: AnyLanePlan): plan is OptimizedLanePlan {
    return 'route' in plan;
}

type CalculatedDistancePlan = (AnyLanePlan) & {
    distanceRan: number;
    delta: number;
    deltaPercentage: number;
};

type TrackSplit = 1 | 2 | 4;
...
const unitMap: Record<string, DistanceUnit> = {
...
const unitPluralSuffixMap: Record<DistanceUnit, string> = {
    [DistanceUnit.Meters]: 'eters',
    [DistanceUnit.Kilometers]: 's',
    [DistanceUnit.Miles]: 'les'
} as const;

type Distance<U extends DistanceUnit> = {
...
// EXTERNAL INTERFACES
// =============================================================================
declare const outputbody: HTMLTableSectionElement;
declare const distance: HTMLInputElement;
declare const split: HTMLSelectElement;
declare const unit: HTMLSelectElement;
declare const distanceinput: HTMLFormElement;
declare const optimize: HTMLInputElement;
...
const renderAndCalculateBestLane = () => {
    ...
    const unitType = unitMap[unit.value as keyof typeof unitMap];
    if (!unitType) {
    ...
    const tracksplit = inputSplit as TrackSplit;
    ...
    for (const [idx, lane] of lanes.entries()) {
        ...
        let plan: AnyLanePlan = createLanePlan(lane, inputDistance, tracksplit);
        if (optimize.checked) {
...
function findNearestSplit(targetDistance: Distance<DistanceUnit.Meters>, forLane: Lane, trackSplit: TrackSplit): number {
...
function createLanePlan(lane: Lane, distance: Distance<DistanceUnit.Meters>, trackSplit: TrackSplit): LanePlan {
    return {
        distance,
        lane,
...
function lanePlanDelta(plan: LanePlan): number {
...
function optimizeLanePlan(lanePlan: LanePlan): OptimizedLanePlan {
    ...
    assert(
        lanePlan.lane.number === 1 || deltaLeading !== deltaTrailing,
        `Delta leading and trailing should not be equal for lane ${lanePlan.lane.number}.`
    );
    ...
    let routePlan = chosenIndex as RoutePlan;
...
function calculateDistancePlan(plan: AnyLanePlan): CalculatedDistancePlan {
    ...
    if (!isOptimized(plan) || plan.route === RoutePlan.FullLane) {
        return {
            ...
            distanceRan,
            delta,
            deltaPercentage: (delta / plan.distance.value) * 100,
        };
    }
